...
const ZapRouterModal: React.FC<ZapProps> = ({ onConfirm, onDismiss, tokenName = '', decimals = 18 }) => {
  ...
  const [zappingTokenBalance, setZappingTokenBalance] = useState(
    getDisplayBalance(tokenName.startsWith(TOMB_TICKER) ? tombBalance : tshareBalance, decimals),
  );
  ...
  const handleSelectFtmMax = async () => {
    ...
    const tokenVal =
      Number(zappingFtmTokenBalance) / Number(tokenName.startsWith(TOMB_TICKER) ? tombPriceInFTM : tSharePriceInFTM);
    setVal(tokenVal.toString());
    ...
    <Modal>
      ...
      {Number(val) > Number(zappingTokenBalance) && (
        <Alert variant="filled" severity="error">
          {' '}
          {Number(val) > Number(zappingTokenBalance)
            ? 'Insufficient ' + (tokenName.startsWith(TOMB_TICKER) ? TOMB_TICKER : TSHARE_TICKER)
            : ''}{' '}
        </Alert>
      )}
        ...
        <Button
          ...
          onClick={() =>
            approveZapperStatus !== ApprovalState.APPROVED
              ...
              ? approveFtmZapper()
              : onConfirm(zappingToken, val, ftmVal, (Number(estimate.token1) / Number(ftmAmountPerLP)).toString())
          }
        >
          {approveZapperStatus !== ApprovalState.APPROVED
